...
SELECT models.name AS model_name, brands.name AS brand_name, brands.headquarters
FROM models
...
FROM models AS m
LEFT JOIN awards AS a ON (m.model_id = a.winner_id)
WHERE m.year > 1963;
...
FROM models as m
LEFT JOIN brands as b ON m.brand_id = b.brand_id
...
SELECT m.name, m.year FROM models AS m
